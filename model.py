...
class Critic():
    ...
    def build_critic(self, name):
        state_input = keras.layers.Input(shape=self.observation_space, name='state_input')
        action_input = keras.layers.Input(shape=self.action, name='action_input')
        ...
        model = keras.Model(inputs=[state_input, action_input], outputs=output, name=name)
...
class Agent():
    ...
    def train(self, batch_size = 64):
        ...
        s, a, s_, r, t =  self.get_batch(batch_size)

        #Train critic
        a_ = self.actor.target_model(s_, verbose = 0)
        next_q = self.critic.target_model([s_, a_], verbose = 0)
        target_q = r + self.gamma*next_q

        with tf.GradientTape() as tape:
            curr_q = self.critic.model([s,a], verbose = 0)
        ...
        self.critic.model.optimizer.apply_gradient(zip(critic_grad, self.critic.model.trainable_variables))

        #Train actor
        with tf.GradientTape() as tape:
            action = self.actor.model(s, verbose = 0)
            actor_loss = tf.reduce_mean(-curr_q)

        actor_grad = tape.gradient(actor_loss, self.actor.model.trainable_variables)

        self.actor.model.optimizer.apply_gradient(zip(actor_grad, self.actor.model.trainable_variables))
